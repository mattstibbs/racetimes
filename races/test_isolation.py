...
def build(club, prefix, names, series_name, owner, shared):
    ...
    invitation = ClubInvitation.objects.create(club=club, email=f"{prefix.lower()}.invited@example.com")
    return {"club": club, "series": series, "entries": entries, "races": [race_1, race_2], "requests": requests,
            "membership": waiting.memberships.get(), "invitation": token_for(invitation)}
...
def test_harbours_ids_are_not_found_at_demo_club(client, clubs, role):
    ...
    for name, args in harbour_urls.items():
        ...
        for method in (client.get, client.post):
            ...
            if response.status_code == 403 or (response.status_code == 302 and "login" in response["Location"]):
                ...
                continue
            # Another club's invitation is a link that doesn't work here: the same 400
            # page as an expired one, naming nobody.
            not_found = 400 if name == "races:accept_invitation" else 404
            assert response.status_code in (not_found, 405), (name, method.__name__, response.status_code)
            if response.status_code == not_found:
                assert leaks(response.content.decode()) == [] and "black.invited" not in response.content.decode()
                checked.append(name)
    ...
    assert "races:accept_invitation" in checked  # Harbour's invitation, at Demo Club
